...
Public Class reports
    ...
    Private Sub FillPara()
        ...
        End If
        Dim DT As DataTable
        DT = PinnaclePlus.SQLData.GeneralOperations.ExecuteSelect(String.Format("select * from T_Report_Para where REP_ID={0} order by Order_", ddlReports.SelectedValue))
    ...
    Private Sub ViewXL(XL As Boolean)
        Try
            ...
            Dim Dat As Date
            DR = PinnaclePlus.SQLData.GeneralOperations.ExecuteSelectSingleRow(String.Format("select [REP_ID],[Name],[Query],[Description],isnull(Is_PP,0) as Is_PP  from T_Report where REP_ID={0}", ddlReports.SelectedValue))
            Query = DR.Item("Query")
            DT = PinnaclePlus.SQLData.GeneralOperations.ExecuteSelect(String.Format("select * from T_Report_Para where REP_ID={0} order by Order_", ddlReports.SelectedValue))
            For i = 0 To DT.Rows.Count - 1
            ...
            Else
                ...
                GridView1.DataBind()
            End If
            PinnaclePlus.SQLData.Reports.P_Users_Report_UseCount(PinnaclePlus.Security.CurrentUser.ID, ddlReports.SelectedItem.Value)
